...
import {
  getSupportPricings,
  createSupportPricing,
  updateSupportPricing,
  deleteSupportPricing,
  type SupportPricing,
  type CreateSupportPricingDto,
  type UpdateSupportPricingDto,
} from "../../../services/apiSupportPricings";
import { SUPPORT_SERVICE_PAGES } from "../../config/supportServicePages";

const SupportPricingsManagement: React.FC = () => {
  const [pricings, setPricings] = useState<SupportPricing[]>([]);
  const [selectedPage, setSelectedPage] = useState<number>(1);
  const [isLoading, setIsLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingPricing, setEditingPricing] = useState<SupportPricing | null>(
    null
  );
  ...
  const fetchPricings = async () => {
    setIsLoading(true);
    try {
      const data = await getSupportPricings(selectedPage);
      setPricings(data);
    } catch (error) {
  ...
  const handleOpenEditModal = (pricing: SupportPricing) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingPricing) {
        ...
        const updateData: UpdateSupportPricingDto = {
        ...
        const updatedPricing = await updateSupportPricing(
          editingPricing.id,
          updateData
        );
        setPricings(
        ...
        // Create new pricing
        const createData: CreateSupportPricingDto = {
          page_number: formData.page_number,
          title_ar: formData.title_ar,
          title_en: formData.title_en,
          price: formData.price,
        ...
        const newPricing = await createSupportPricing(createData);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await deleteSupportPricing(id);
      setPricings(pricings.filter((p) => p.id !== id));
      toast.success("تم حذف السعر بنجاح");
    } catch (error) {
      console.error("Error deleting support pricing:", error);
      const errorMessage =
...
export default SupportPricingsManagement;
