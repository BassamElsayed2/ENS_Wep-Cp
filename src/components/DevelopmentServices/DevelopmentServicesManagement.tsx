...
import {
  ...
  updateDevelopmentService,
  UpdateDevelopmentServiceDto,
  CreateDevelopmentServiceDto,
} from "../../../services/apiDevelopmentServices";
import { DEVELOPMENT_SERVICE_PAGES } from "../../config/developmentServicePages";

const DevelopmentServicesManagement: React.FC = () => {
  const [services, setServices] = useState<DevelopmentService[]>([]);
  const [selectedPage, setSelectedPage] = useState<number>(1);
  const [isLoading, setIsLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingService, setEditingService] =
    useState<DevelopmentService | null>(null);
  ...
  const handleOpenEditModal = (service: DevelopmentService) => {
  ...
  const handleSubmit = async () => {
    ...
    const loadingToast = toast.loading(
      editingService
        ? "جاري تحديث خدمة البرمجة والتطوير..."
        : "جاري إضافة خدمة برمجة وتطوير..."
    );
    try {
      if (editingService) {
        const updateData: UpdateDevelopmentServiceDto = {
        ...
        const updated = await updateDevelopmentService(
          editingService.id,
          updateData
        );
        setServices(
        ...
        toast.success("تم تحديث خدمة البرمجة والتطوير بنجاح");
      } else {
        const payload: CreateDevelopmentServiceDto = {
          page_number: formData.page_number,
          title_ar: formData.title_ar,
          title_en: formData.title_en,
          description_ar: formData.description_ar,
        ...
        const created = await createDevelopmentService(payload);
        setServices([...services, created]);
        toast.dismiss(loadingToast);
        toast.success("تمت إضافة خدمة البرمجة والتطوير بنجاح");
      }
      setShowModal(false);
      fetchServices();
    } catch (error: unknown) {
      ...
      <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
        ...
        <div className="trezo-card-content">
          <div className="mb-[20px] flex gap-[10px] flex-wrap">
            {DEVELOPMENT_SERVICE_PAGES.map((page) => (
              <button
                key={page.id}
          ...
          <div className="mb-[20px]">
            <div className="relative">
              <input
                type="text"
                placeholder="البحث في خدمات البرمجة والتطوير..."
                value={searchTerm}
                  ...
                  <div className="flex items-center gap-[15px]">
                    ...
                    <div>
                      <h5 className="text-xl font-bold !text-white mb-0">
                        {editingService
                          ? "تعديل خدمة برمجة وتطوير"
                          : "إضافة خدمة برمجة وتطوير"}
                      </h5>
                      <p className="text-white/80 text-sm mt-1">
                        {editingService
                          ? "قم بتحديث معلومات خدمة البرمجة والتطوير"
                          : "أضف خدمة برمجة وتطوير جديدة"}
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <div className="p-[25px]">
                  <div className="space-y-[20px]">
                    <div>
                      ...
                      >
                        {DEVELOPMENT_SERVICE_PAGES.map((p) => (
                    ...
                    <div>
                      <label className="block text-sm font-semibold mb-2 text-gray-700 dark:text-gray-300">
                        صورة خدمة البرمجة والتطوير {!editingService && "*"}
                      </label>
...
export default DevelopmentServicesManagement;
