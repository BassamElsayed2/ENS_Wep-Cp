...
import {
  ...
  deleteAppVersionService,
  AppVersionService,
  getAppVersionServices,
  updateAppVersionService,
  UpdateAppVersionServiceDto,
  CreateAppVersionServiceDto,
} from "../../../services/apiAppVersionServices";
import { APP_VERSION_PAGES } from "../../config/appVersionPages";

const AppVersionServicesManagement: React.FC = () => {
  const [services, setServices] = useState<AppVersionService[]>([]);
  const [selectedPage, setSelectedPage] = useState<number>(1);
  const [isLoading, setIsLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingService, setEditingService] =
    useState<AppVersionService | null>(null);
  ...
  const handleOpenEditModal = (service: AppVersionService) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!editingService && !selectedImage) {
      toast.error("الرجاء اختيار صورة للإصدار البرمجي");
      return;
    }

    const loadingToast = toast.loading(
      editingService
        ? "جاري تحديث الإصدار البرمجي..."
        : "جاري إضافة إصدار برمجي..."
    );
    try {
      if (editingService) {
        const updateData: UpdateAppVersionServiceDto = {
        ...
        const updated = await updateAppVersionService(
          editingService.id,
          updateData
        );
        setServices(
        ...
        toast.success("تم تحديث الإصدار البرمجي بنجاح");
      } else {
        const payload: CreateAppVersionServiceDto = {
          page_number: formData.page_number,
          title_ar: formData.title_ar,
          title_en: formData.title_en,
          description_ar: formData.description_ar,
        ...
        const created = await createAppVersionService(payload);
        setServices([...services, created]);
        toast.dismiss(loadingToast);
        toast.success("تمت إضافة الإصدار البرمجي بنجاح");
      }
      setShowModal(false);
      fetchServices();
    } catch (e) {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("هل أنت متأكد من حذف هذا الإصدار البرمجي؟")) return;
    try {
        ...
        <div className="trezo-card-content">
          ...
          <div className="mb-[20px]">
            <div className="relative">
              <input
                type="text"
                placeholder="البحث في الإصدارات البرمجية..."
                value={searchTerm}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12 text-gray-500 dark:text-gray-400">
              {searchTerm
                ? "لا توجد إصدارات برمجية تطابق البحث"
                : "لا توجد إصدارات برمجية في هذه الصفحة"}
            </div>
          ) : (
              ...
              {totalPages > 1 && (
                <div className="mt-[20px] px-[20px] py-[15px] border-t border-gray-200 dark:border-[#172036] flex items-center justify-between">
                  <div className="text-sm text-gray-700 dark:text-gray-300">
                    ...
                    {filteredServices.length} إصدار برمجي
                  </div>
                  <div className="flex gap-[5px]">
                    <button
                      onClick={() =>
              ...
              <div className="w-full max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="flex items-center gap-[15px]">
                    ...
                    <div>
                      ...
                      <p className="text-white/80 text-sm mt-1">
                        {editingService
                          ? "قم بتحديث معلومات الإصدار البرمجي"
                          : "أضف إصدار برمجي جديد"}
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <div className="p-[25px]">
                  <div className="space-y-[20px]">
                    ...
                    <div>
                      <label className="block text-sm font-semibold mb-2 text-gray-700 dark:text-gray-300">
                        صورة الإصدار البرمجي {!editingService && "*"}
                      </label>
                      <div className="border-2 border-dashed border-gray-300 dark:border-[#172036] rounded-lg p-[20px] text-center hover:border-primary-500 transition-colors">
                        <input
                          ...
                          className="hidden"
                          id="app-version-image-upload"
                        />
                        <label
                          htmlFor="app-version-image-upload"
                  ...
                  <div className="flex gap-[15px] mt-[30px] pt-[25px] border-t border-gray-200 dark:border-[#172036]">
                    ...
                    >
                      ...
                      {editingService ? "تحديث الإصدار" : "إضافة الإصدار"}
                    </button>
                    <button
                      onClick={() => setShowModal(false)}
...
export default AppVersionServicesManagement;
