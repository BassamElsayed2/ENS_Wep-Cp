...
import {
  ...
  UpdateSupportServiceDto,
  CreateSupportServiceDto,
} from "../../../services/apiSupportServices";
import { SUPPORT_SERVICE_PAGES } from "../../config/supportServicePages";

const SupportServicesManagement: React.FC = () => {
  const [services, setServices] = useState<SupportService[]>([]);
  const [selectedPage, setSelectedPage] = useState<number>(1);
  const [isLoading, setIsLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingService, setEditingService] = useState<SupportService | null>(
    null
  );
  ...
  const handleOpenEditModal = (service: SupportService) => {
  ...
  const handleSubmit = async () => {
    ...
    const loadingToast = toast.loading(
      editingService
        ? "جاري تحديث خدمة الدعم الفني..."
        : "جاري إضافة خدمة دعم فني..."
    );
    try {
      if (editingService) {
        const updateData: UpdateSupportServiceDto = {
        ...
        const updated = await updateSupportService(
          editingService.id,
          updateData
        );
        setServices(
        ...
        toast.success("تم تحديث خدمة الدعم الفني بنجاح");
      } else {
        const payload: CreateSupportServiceDto = {
          page_number: formData.page_number,
          title_ar: formData.title_ar,
          title_en: formData.title_en,
          description_ar: formData.description_ar,
        ...
        const created = await createSupportService(payload);
        setServices([...services, created]);
        toast.dismiss(loadingToast);
        toast.success("تمت إضافة خدمة الدعم الفني بنجاح");
      }
      setShowModal(false);
      fetchServices();
    } catch (error: unknown) {
      ...
      const msg =
        error instanceof Error
          ? error.message
          : "حدث خطأ أثناء حفظ خدمة الدعم الفني";
      toast.error(msg);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      const msg =
        error instanceof Error
          ? error.message
          : "حدث خطأ أثناء حذف خدمة الدعم الفني";
      toast.error(msg);
    }
  };
      ...
      <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
        ...
        <div className="trezo-card-content">
          <div className="mb-[20px] flex gap-[10px] flex-wrap">
            {SUPPORT_SERVICE_PAGES.map((page) => (
              <button
                key={page.id}
          ...
          <div className="mb-[20px]">
            <div className="relative">
              <input
                type="text"
                placeholder="البحث في خدمات الدعم الفني..."
                value={searchTerm}
                      ...
                      >
                        <td className="px-[20px] py-[15px] whitespace-nowrap">
                          <div className="relative h-12 w-12 bg-gray-100 dark:bg-[#15203c] rounded-md overflow-hidden">
                            <Image
                              src={service.img}
                              alt={service.titleEn || "Support Service"}
                              fill
              ...
              <div className="w-full max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="flex items-center gap-[15px]">
                    ...
                    <div>
                      ...
                      <p className="text-white/80 text-sm mt-1">
                        {editingService
                          ? "قم بتحديث معلومات خدمة الدعم الفني"
                          : "أضف خدمة دعم فني جديدة"}
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <div className="p-[25px]">
                  <div className="space-y-[20px]">
                    ...
                    <div>
                      <label className="block text-sm font-semibold mb-2 text-gray-700 dark:text-gray-300">
                        صورة خدمة الدعم الفني {!editingService && "*"}
                      </label>
...
export default SupportServicesManagement;
