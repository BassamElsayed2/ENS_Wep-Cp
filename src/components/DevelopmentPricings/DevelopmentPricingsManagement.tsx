...
import {
  getDevelopmentPricings,
  createDevelopmentPricing,
  updateDevelopmentPricing,
  deleteDevelopmentPricing,
  type DevelopmentPricing,
  type CreateDevelopmentPricingDto,
  type UpdateDevelopmentPricingDto,
} from "../../../services/apiDevelopmentPricings";
import { DEVELOPMENT_SERVICE_PAGES } from "../../config/developmentServicePages";

const DevelopmentPricingsManagement: React.FC = () => {
  const [pricings, setPricings] = useState<DevelopmentPricing[]>([]);
  const [selectedPage, setSelectedPage] = useState<number>(1);
  const [isLoading, setIsLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingPricing, setEditingPricing] =
    useState<DevelopmentPricing | null>(null);
  ...
  const handleOpenEditModal = (pricing: DevelopmentPricing) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingPricing) {
        ...
        const updateData: UpdateDevelopmentPricingDto = {
        ...
        const updatedPricing = await updateDevelopmentPricing(
          editingPricing.id,
          updateData
        );
        setPricings(
        ...
        // Create new pricing
        const createData: CreateDevelopmentPricingDto = {
          page_number: formData.page_number,
          title_ar: formData.title_ar,
          title_en: formData.title_en,
          price: formData.price,
        ...
        const newPricing = await createDevelopmentPricing(createData);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await deleteDevelopmentPricing(id);
        ...
        <div className="trezo-card-header mb-[20px] md:mb-[25px] flex items-center justify-between">
          <div className="trezo-card-title">
            <h5 className="!mb-0 text-xl font-bold text-black dark:text-white">
              إدارة أسعار خدمات البرمجة والتطوير
            </h5>
          </div>
          <button
...
export default DevelopmentPricingsManagement;
